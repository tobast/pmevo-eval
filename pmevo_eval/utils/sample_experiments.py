...
from pmevo_eval.processors.remote_processor import RemoteProcessor
...
from pmevo_eval.utils.jsonable import Vault
...
def length_probability(U, length, minl, maxl):
    ...
    length.
    """
    return num_multicomb(U, length) / sum(
        (num_multicomb(U, i) for i in range(minl, maxl))
    )
...
def compute_patterns(num, l):
    ...
    for p in all_patterns:
        ...
        num_kperm = binomial(num, num_distinct_placeholders) * fac(
            num_distinct_placeholders
        )
