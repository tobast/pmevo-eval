...
from pmevo_eval.utils.experiment import ExperimentList
...
class Inferrer(ABC):
    ...
    def make_partitioning(cls):
        ...
        """
        from utils.partition_insns import (
        ...
        class PartitioningInferrer(cls):
            ...
            def infer(self, exps):
                ...
                reps, insn_to_rep = compute_representatives(
                    complex_exps, singleton_exps, epsilon=self.epsilon
                )

                exps = restrict_elist(exps, reps)
                print(
                    "Restricted input to {insns} out of {old_insns} instructions and {exps} out of {old_exps} experiments.".format(
                        insns=len(reps),
...
class EvoAlgoWrapper(Inferrer):
    ...
    def __init__(self, config=None):
        ...
        self.cmd = [
            ...
            "-c{}".format(
                self.config_path
            ),  # use the specified config for the genetic algorithm
    ...
    def infer(self, exps):
        ...
        cmd = self.cmd + [
            "-q{}".format(len(exps.arch.ports)),
            "-e{}".format(singleton_elist_path),
        ]
        ...
        if retval != 0:
            print(
                "Evolutionary algorithm binary returned with non-zero return code: {}!".format(
                    retval
                ),
                file=sys.stderr,
            )
...
def evaluateExperiments(bin_path, m, exps, num_repetitions):
    ...
    if retval != 0:
        print(
            "Genetic algorithm binary returned with non-zero return code!",
            file=sys.stderr,
        )
        print("output on stdout:", file=sys.stderr)
        print(textwrap.indent(out_str, "  "), file=sys.stderr)
