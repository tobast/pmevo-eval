...
from pmevo_eval.utils.mapping import Mapping3
...
def partition_instructions(elist, singleton_elist, epsilon, verbose=False, stats=None):
    ...
    singleton_equiv_map = {
        (i, j): equals(singleton_results[i], singleton_results[j])
        ...
        for i in insns:
            ...
            for e1, e2 in zip(i1i_exps, i2i_exps):
                if not (len(e1.iseq) == len(e2.iseq)):
                    ...
                    if verbose:
                        print(
                            "Warning: Corresponding experiments with differing length!",
                            file=sys.stderr,
                        )
                ...
                if not equals(e1.get_cycles(), e2.get_cycles()):
                    ...
                    if verbose:
                        print(
                            "distinguishing experiments for {} and {}:".format(i1, i2)
                        )
