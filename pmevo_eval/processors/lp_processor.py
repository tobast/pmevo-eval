...
from pmevo_eval.utils.architecture import Architecture, Insn, Port
from pmevo_eval.utils.experiment import Experiment
...
class LPProcessor(SimProcessor):
    ...
    def cycles_for_weights(self, weights):
        ...
        x_vars = m.addVars([(i, k) for i in weights.keys() for k in P], name="x")

        lat = m.addVar(name="latency", obj=1.0)

        m.addConstrs(
            (quicksum([x_vars[(i, k)] for k in P]) == n for i, n in weights.items())
        )

        m.addConstrs(
            (quicksum([x_vars[(i, k)] for i in weights.keys()]) <= lat for k in P)
        )
